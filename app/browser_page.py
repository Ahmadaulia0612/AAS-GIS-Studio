...
class BrowserPage(QWidget):
    ...
    def init_ui(self):
        ...
        # Baris 3: Export KML & Calculate Energy
        row3 = QHBoxLayout()
        ...
        self.btn_calc_energy = QPushButton("Energy")
        self.btn_calc_energy.setStyleSheet("background-color: #17a2b8; color: white; font-weight: bold;")
    ...
    def load_dem(self):
        ...
        for f in new_files:
            self.add_dem_item_with_x(f, os.path.basename(f))

        self.update_dem_bounds_on_map()
        QMessageBox.information(self, "DEM", f"{len(new_files)} file DEM baru berhasil ditambahkan.")
    ...
    def remove_single_dem(self, file_path, item):
        ...
        self.update_dem_bounds_on_map()
        QMessageBox.information(self, "DEM Dihapus", f"File {os.path.basename(file_path)} berhasil dihapus.")
    ...
    def load_river(self):
        filename, _ = QFileDialog.getOpenFileName(self, "Pilih River", "", "GeoPackage (*.gpkg *.shp)")
        if not filename:
            return

        self.river_file = filename
        self.add_river_item_with_x(filename, os.path.basename(filename))

        QMessageBox.information(self, "Memuat River", "File sungai sedang dimuat di latar belakang...")
    ...
    def add_river_item_with_x(self, file_path, file_name):
        river_node = None
        for i in range(self.project.topLevelItemCount()):
            top_item = self.project.topLevelItem(i)
            if top_item.text(0).strip().upper() == "RIVER":
                river_node = top_item
                break
            for j in range(top_item.childCount()):
                child = top_item.child(j)
                if child.text(0).strip().upper() == "RIVER":
                    river_node = child
                    break

        target_parent = river_node if river_node else self.project.topLevelItem(0)
        while target_parent.childCount() > 0:
            target_parent.removeChild(target_parent.child(0))
    ...
    def remove_single_river(self, file_path, item):
        self.river_file = None
        parent = item.parent()
        if parent:
            parent.removeChild(item)
        self.page.loadRiver({'type': 'FeatureCollection', 'features': []})
        QMessageBox.information(self, "River Dihapus", "File sungai berhasil dihapus dari daftar.")

    def on_river_loaded(self, geojson_dict):
        if geojson_dict:
            self.page.loadRiver(geojson_dict)
        QMessageBox.information(self, "River", "Data sungai berhasil ditampilkan di peta!")

    def on_river_error(self, err_msg):
        QMessageBox.critical(self, "Error River", f"Gagal membaca file River:\n{err_msg}")

    def load_outlet_kml(self):
        filename, _ = QFileDialog.getOpenFileName(self, "Pilih File KML/KMZ", "", "KML/KMZ (*.kml *.kmz)")
        if not filename:
            return
        try:
            ...
            if not gdf.empty:
                ...
                self.outlet = (lat, lon)
                self.btn_nasa.setEnabled(True)
                self.page.runJavaScript(f"setOutletMarker({lat}, {lon});")
                QMessageBox.information(self, "Outlet KML", f"Titik outlet dimuat!\nLat: {lat}, Lon: {lon}")
        except Exception as e:
    ...
    def coordinate_received(self, lat, lon):
        self.outlet = (lat, lon)
        self.btn_nasa.setEnabled(True)
        QMessageBox.information(self, "Outlet Terpilih", f"Outlet diatur:\nLat: {lat:.5f}\nLon: {lon:.5f}")
    ...
    def export_watershed_kml(self):
        ...
        if not os.path.exists(watershed_geojson_path):
            QMessageBox.warning(self, "Export KML", "Belum ada data Watershed.")
            return

        filename, _ = QFileDialog.getSaveFileName(self, "Simpan KML", "watershed.kml", "KML Files (*.kml)")
        if not filename:
            return
        ...
        try:
            ...
            QMessageBox.information(self, "Export KML Berhasil", f"Disimpan di:\n{filename}")
        except Exception as e:
            QMessageBox.critical(self, "Error Export", f"Gagal:\n{str(e)}")

    def open_energy_calculator(self):
        from app.widget.energy_dialog import EnergyCalculatorDialog
        dialog = EnergyCalculatorDialog(self)
        dialog.exec()

    def open_nasa_dialog(self):
    ...
    def run_watershed(self):
        if not self.dem_files or not self.river_file or self.outlet is None:
            QMessageBox.warning(self, "Hydrology", "Pastikan DEM, River, dan Outlet sudah dipilih.")
            return

        self.btn_ws.setEnabled(False)
        self.btn_ws.setText("Memproses...")

        self.worker = HydrologyWorker(dem_files=self.dem_files, river_file=self.river_file, outlet=self.outlet)
        self.worker.progress_signal.connect(lambda p, m: print(f"[{p}%]: {m}"))
    ...
    def on_watershed_finished(self, message, duration, area_km2):
        ...
        self.calculated_ca_km2 = area_km2

        self.btn_export_kml.setEnabled(True)
        self.btn_calc_energy.setEnabled(True)
